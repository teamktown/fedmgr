...
Status: Phases 0–5 complete (2026-06-11); Phase 6 in progress (6.0 done 2026-06-12); Phase 7 specced — not started
...
| 6 | MCP demonstrations (E1/E8/E9, **6.4 Waypoint multiplexer**, E5a/E5b) | 🚧 **6.0, 6.1, 6.4 done + waypoint HTTP/SSE + OAuth edge (identity plane) done** (`docs/evidence/phase-6.4-waypoint-http-oauth.md`: remote MCP over Streamable HTTP, bearer-gated, 401 fail-closed, real client initialize+tools — inbound OIDC, outbound OIDF). 6.0 tool + OTel; 6.1 `oidf-trust` extension + accepted-anchor + digest binding + **full lab + round-trip end-to-end in the VM** (`docs/evidence/phase-6.1-lab-end-to-end.md`). **6.4 = `@letsfederate/waypoint`**: a trust-enforcing MCP multiplexer — client points at one server, it gates downstream MCPs and exposes only trusted tools (namespaced). Two layers: accepted-anchor config policy **and cryptographic admission** (`requireValidChain` → `validateTrustChain` must resolve VALID). Proven against the live lab: VALID chain → 12 tools, INVALID/non-accepted → 0 (`docs/evidence/phase-6.4-waypoint-test-evidence.md`, `…-crypto-admission.md`). Still ahead: continuous revocation re-check. E8 next |
