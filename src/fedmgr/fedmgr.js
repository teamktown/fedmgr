...
program
  ...
  .option(
    '--inspector-args <args...>',
    'Additional arguments to pass to the MCP Inspector executable (e.g., --method tools/list)',
  )
  .description('Launch MCP Inspector to connect to a running MCP instance')
  .action((mcpName, opts) => {
    const registry = loadRegistry()
    const mcpPort = registry.mcps?.[mcpName]

    if (!mcpPort) {
      console.error(
        `❌ MCP instance '${mcpName}' not found or its port is not defined in registry.`,
      )
      console.error(`   Available MCPs in registry: ${JSON.stringify(registry.mcps)}`)
      process.exit(1)
    }
    ...
    // Add passthrough arguments for the inspector program itself
    // These can be general flags or action-specific flags like --method
    if (opts.inspectorArgs && opts.inspectorArgs.length > 0) {
      finalInspectorArgs.push(...opts.inspectorArgs)
    }

    // Add the target MCP URL as the last primary argument
    finalInspectorArgs.push(mcpUrl)

    const envVars = { ...process.env }
    if (!opts.cli && opts.inspectorUiPort !== '6274') {
    ...
    if (!opts.cli) {
      ...
      console.log(
        `   Note: If fedmgr is running in Docker, ensure port ${opts.inspectorUiPort} is mapped from the container.`,
      )
    }
