...
# (gitignored except the report) and folded into a single markdown report
# at SSC/report<YYYYMMDD>.v<N>.md with stub Assessment + Action sections
...
#
# Usage:
#   scripts/scan-all.sh                # runs all available scanners
#   scripts/scan-all.sh --image IMG    # also runs Trivy against an image (optional)
#   scripts/scan-all.sh --quiet        # suppress per-tool stdout (still writes logs)
#
# Exit codes:
...
# Determine the next version for today: walk SSC/report${DATE}.v*.md, find
# the largest <N>, increment. New day = v1.
NEXT_N=1
for existing in "$SSC_DIR"/report"$DATE".v*.md; do
  [ -e "$existing" ] || break
  n="$(basename "$existing" | sed -E "s/^report${DATE}\.v([0-9]+)\.md$/\1/")"
  [[ "$n" =~ ^[0-9]+$ ]] && [ "$n" -ge "$NEXT_N" ] && NEXT_N=$((n + 1))
done
REPORT="$SSC_DIR/report${DATE}.v${NEXT_N}.md"
...
> This report is generated by \`scripts/scan-all.sh\`. The **Assessment**
...
if [ "${#MISSING_TOOLS[@]}" -gt 0 ]; then
  ...
  printf "  ${BLU}scripts/install-sectools.sh${RST}                   ${DIM}# read the consent banner${RST}\n" >&2
  printf "  ${BLU}scripts/install-sectools.sh --confirm-install-sectools${RST}\n\n" >&2
...
run_tool hadolint "$HADOLINT_OUT" \
  hadolint --format json Dockerfile || true
